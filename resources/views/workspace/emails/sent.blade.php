@extends('layouts.email', ['pageTitle' => 'الرسائل الصادرة'])

@section('content')
    <div class="space-y-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div class="flex flex-wrap items-center gap-2">
                <h2 class="text-base font-bold text-slate-900">البريد الصادر</h2>
                <span class="rounded-full bg-[#E8FAF6] px-2 py-1 text-xs font-semibold text-[#0f7668]">{{ $messages->total() }} رسالة</span>
            </div>
            <form method="GET" action="{{ route('workspace.emails.sent') }}" class="mt-3 grid gap-3 md:grid-cols-3">
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex gap-2">
                        <input type="text" name="search" value="{{ $search }}" placeholder="ابحث بالعنوان أو المستلم أو المحتوى"
